(function () {
  ...
  function renderStoredMaze(isSolution) {
    ...
    const mazeData = MBG.lastPreviewMaze;
    const settings = readSettings();

    canvas.width = MBG.PAGE_W || 2550;
    canvas.height = MBG.PAGE_H || 3300;
    ...
    document.querySelectorAll("[data-maze-preview-lock-mode]").forEach(function (button) {
      const active = button.dataset.mazePreviewLockMode === (isSolution ? "solution" : "puzzle");
  ...
  function installButtons() {
    ...
    controls.setAttribute("aria-label", "Podgląd tej samej pary labiryntu");

    const label = document.createElement("strong");
    label.textContent = "Ten sam układ:";

    const note = document.createElement("span");
    note.textContent = "Przełączanie nie losuje nowego labiryntu.";

    controls.append(
      label,
      createButton("puzzle", "Pokaż zadanie"),
      createButton("solution", "Pokaż rozwiązanie"),
      note
    );
    ...
    if (!el("mazePreviewLockStyles")) {
      ...
      style.textContent = [
        ".maze-preview-lock-controls{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 auto 12px;max-width:760px;padding:10px 12px;border:1px solid rgba(99,220,255,.34);border-radius:9px;background:rgba(8,16,28,.82)}",
        ".maze-preview-lock-controls strong{color:#f8fafc}",
        ".maze-preview-lock-controls span{color:#aeb7c7;font-size:12px}",
        ".maze-preview-lock-button{padding:8px 12px;font-size:12px;box-shadow:none}",
  ...
  function protectExistingPreviewSelector() {
    ...
    selector.addEventListener("change", function (event) {
      ...
      if (typeof MBG === "undefined" || !MBG.lastPreviewMaze) return;

      event.preventDefault();
      event.stopImmediatePropagation();
      renderStoredMaze(isSolutionPreviewType(value));
    }, true);
  }
  ...
  function initialize() {
    ...
    const tryInstall = function () {
      ...
      if (!ready && attempts < 80) window.setTimeout(tryInstall, 50);
    };
    tryInstall();
  }
